...
完全ローカル音声認識（`speech_recognition.engine: sphinx`）を使う場合のみ、追加で以下をインストール:
...
1. VOICEVOX Engine をインストール・起動（https://voicevox.hiroshiba.jp/）
...
- `speech_recognition.engine`
  - 音声認識エンジン。`google`（既定・オンライン）/ `sphinx`（完全ローカル）
...
- マイクが開けない
  ...
  - OS のマイク許可設定を確認
- 音声認識 API エラー
  - `speech_recognition.engine: google` の場合はネットワーク状態を確認
  - `speech_recognition.engine: sphinx` の場合は `pocketsphinx` 導入と音響モデル設定を確認
  - `language` 設定が意図通りか確認
- AI 応答が取れない
  - `ai.api_url` が正しいか確認
  - AI サーバー起動状態を確認
